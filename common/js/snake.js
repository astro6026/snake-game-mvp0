...
const nextSnake = state => {
    ...
    return [nextHead(state)].concat(base.dropLast(state.snake))
}
...
const enqueue = (state, move) => {
    ...
    if (validMove(move)(state))
        return base.merge(state)({ moves: state.moves.concat([move]) })
    return state
}
module.exports = {
    initialState, enqueue, next, NORTH, EAST, WEST, SOUTH, 
}
